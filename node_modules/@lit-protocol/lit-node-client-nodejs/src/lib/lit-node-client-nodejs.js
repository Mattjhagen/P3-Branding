...
class LitNodeClientNodeJs extends core_1.LitCore {
    ...
    `);
        ...
        this.runOnTargetedNodes = async (params) => {
            ...
            for (let i = 0; i < randomSelectedNodeIndexes.length; i++) {
                ...
                const singleNodePromise = this.getJsExecutionShares(url, reqBody, requestId);
        ...
        this.getSignatures = (signedData) => {
            (0, misc_1.log)(`getSignatures(): ${JSON.stringify(signedData, null, 2)}`);
            const validatedSignedData = signedData
                ...
                for (const field of requiredFields) {
                    (0, misc_1.log)('Checking sigShare:', sigShare);
            ...
            (0, misc_1.log)('requested length:', signedData.length);
            (0, misc_1.log)('validated length:', validatedSignedData.length);
            (0, misc_1.log)('minimum required length:', this.config.minNodeCount);
            if (validatedSignedData.length < this.config.minNodeCount) {
                (0, misc_1.log)(`not enough nodes to get the signatures.  Expected ${this.config.minNodeCount}, got ${validatedSignedData.length}`);
                return null;
            }
            // -- prepare
            const signatures = {};
            ...
            // -- execute
            keys.forEach((key) => {
                const shares = validatedSignedData.map((r) => r[key]);
                shares.sort((a, b) => a.shareIndex - b.shareIndex);
                ...
                const validatedSigShares = sigShares
                    ...
                    .filter((s) => s !== null);
                (0, misc_1.log)('requested length:', signedData.length);
                (0, misc_1.log)('validated length:', validatedSigShares.length);
                (0, misc_1.log)('minimum required length:', this.config.minNodeCount);
                if (validatedSigShares.length < this.config.minNodeCount) {
                    (0, misc_1.log)(`not enough nodes to get the signatures.  Expected ${this.config.minNodeCount}, got ${validatedSigShares.length}`);
                }
                const sigType = (0, misc_1.mostCommonString)(sigShares.map((s) => s.sigType));
                // -- validate if this.networkPubKeySet is null
                if (this.networkPubKeySet === null) {
        ...
        this.executeJs = async (params) => {
            ...
            (0, misc_1.log)('executeJs responseData', JSON.stringify(responseData, null, 2));
            ...
            (0, misc_1.log)('signedDataList:', signedDataList);
            ...
            (0, misc_1.log)('claimList:', claimsList);
        ...
        this.pkpSign = async (params) => {
            ...
            const nodePromises = this.getNodePromises((url) => {
                ...
                (0, misc_1.log)('sigToPassToNode:', sigToPassToNode);
                let reqBody = {
                    toSign,
                    pubkey: pubKey,
                    ...(sigToPassToNode &&
        ...
        this.signSessionKey = async (params) => {
            ...
            (0, misc_1.log)('handleNodePromises res:', res);
            // -- case: promises rejected
            ...
            const validatedSignedDataList = signedDataList
                ...
                .filter((item) => item !== null);
            (0, misc_1.log)('requested length:', signedDataList.length);
            (0, misc_1.log)('validated length:', validatedSignedDataList.length);
            (0, misc_1.log)('minimum required length:', this.config.minNodeCount);
    ...
    async claimKeyId(params) {
        ...
        const responseData = await this.handleNodePromises(nodePromises, this.connectedNodes.size // require from all connected nodes
        );
        if (responseData.success === true) {
            ...
            (0, misc_1.log)(`pubkey ${pubkey} derived from key id ${derivedKeyId}`);
...
LitNodeClientNodeJs.getClaims = (claims) => {
    let keys = Object.keys(claims[0]);
    let signatures = {};
    let claimRes = {};
    for (let i = 0; i < keys.length; i++) {
        let claimSet = claims.map((c) => c[keys[i]]);
        signatures[keys[i]] = [];
        claimSet.map((c) => {
 ...
 */
LitNodeClientNodeJs.generateSessionCapabilityObjectWithWildcards = (litResources) => {
